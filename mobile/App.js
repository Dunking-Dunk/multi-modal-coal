...
import { setUser, getVehicle, getPlaces } from './store/AuthReducer.js';

const Stack = createStackNavigator()

const StackNavigator = () => {
  const {user, location} = useSelector((state) => state.Main)
    ...
    const helper = async() => {
      try {
        ...
        if (res.data.user.role === 'driver') {
          dispatch(getVehicle(res.data.user._id))
        } else {
          dispatch(getPlaces(res.data.user._id))
        }
